...
<div id="wa-widget" x-data="waWidget('{{ $waNumber }}')" class="fixed bottom-6 right-6 z-999 flex flex-col items-end">
    <!-- Chat Box -->
    <div x-show="open" x-transition @click.away="open = false" class="w-75 sm:w-90 shadow-lg rounded-xl overflow-hidden mb-3 bg-cover bg-center" style="background-image:url('{{ asset('storage/media/wa-background.jpg') }}')">
        ...
        <div class="p-4 bg-white space-y-2 m-6 rounded-md bubble-wa relative">
            ...
            <img class="absolute top-0 -left-3 w-12 z-10" src="{{ asset('storage/media/wa-bubble-caret.png') }}">
            <div class="flex flex-row justify-end gap-2 mt-3"><p class="text-[.8em]">{{ date('H:i') }}</p></div>
        </div>
        <!-- input -->
        <div class="px-6 pb-3">
            <form action="#" @submit.prevent="kirimWA" class="flex flex-row gap-2">  
                ...
                <button type="submit" 
                    ...
                    style="background-image:url('{{ asset('storage/media/wa-btn.png') }}')"
                ></button>
            </form>  
        </div>
    </div>
    ...
    <!-- Chat Button -->
    <button @click="open = !open" class=" w-14 h-14 rounded-full bg-green-500 text-white text-2xl flex items-center justify-center shadow-lg hover:bg-green-600 transition duration-200 cursor-pointer">
